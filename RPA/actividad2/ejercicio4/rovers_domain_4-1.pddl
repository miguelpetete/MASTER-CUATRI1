(define (domain Rover2)
    ...
    (:types
        rover waypoint store camera mode lander level
    )
    ...
    (:action navigate
        ...
        :precondition (and (can_traverse ?x ?y ?z) (available ?x) (at ?x ?y) (visible ?y ?z) (or (battery_full ?x) (battery_medium ?x)))
        :effect (and
            (not (at ?x ?y)) (at ?x ?z)
            (when
                (battery_full ?x)
                (and (not (battery_full ?x)) (battery_medium ?x)))
            (when
                (battery_medium ?x)
                (and (not (battery_medium ?x)) (battery_low ?x)))
        )
    )

    (:action recharge
        :parameters (?x - rover ?l - lander ?w - waypoint)
        :precondition (and (at_lander ?l ?w) (at ?x ?w) (channel_free ?l))
        :effect (and
            (not (battery_low ?x)) (not (battery_medium ?x))
            (battery_full ?x)
        )
    )
